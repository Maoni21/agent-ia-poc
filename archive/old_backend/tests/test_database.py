...
class TestDatabaseValidation:
    ...
    def test_save_workflow_result(self):
        """Test de sauvegarde d'un workflow complet"""
        test_db_path = os.path.join(self.temp_dir, "workflow_test.db")
        config = DEFAULT_DATABASE_CONFIG.copy()
        config['database_path'] = test_db_path
        ...
        # Créer un WorkflowResult minimal pour test
        ...
        # Créer un workflow_result minimal
        workflow_result = WorkflowResult(
            workflow_id="test_workflow_001",
            workflow_type=WorkflowType.SCAN_ONLY,
            target="192.168.1.100",
        ...
        workflow_id = db.save_workflow_result(workflow_result)
        assert workflow_id == "test_workflow_001"
        
        # Vérifier que le workflow a été sauvegardé
        workflow = db.select_one('workflows', where={'workflow_id': workflow_id})
        assert workflow is not None
        assert workflow['target'] == "192.168.1.100"
        
        # Vérifier que le scan a été sauvegardé
        scan = db.select_one('scans', where={'scan_id': scan_result.scan_id})
        assert scan is not None
        
        # Vérifier que la vulnérabilité a été sauvegardée
        vuln = db.select_one('vulnerabilities', where={'vulnerability_id': 'CVE-2024-0001'})
        assert vuln is not None
        assert vuln['severity'] == 'HIGH'
        
        # Vérifier la liaison scan-vulnérabilité
        link = db.select_one('scan_vulnerabilities', where={
