...
class Supervisor:
    ...
    async def _save_workflow_result(self, result: WorkflowResult):
        ...
        try:
            ...
            # 1. Sauvegarder le JSON (log technique)
            ...
            # 2. Sauvegarder dans la base de données (source de vérité)
            try:
                self.db.save_workflow_result(result)
                logger.info(f"✅ Workflow synchronisé en base de données: {result.workflow_id}")
            except Exception as e:
                # Ne pas faire échouer le workflow si la DB échoue
                logger.warning(f"⚠️ Erreur sauvegarde workflow en DB (non-bloquant): {e}")
